...
function generateBashCompletion(): string {
  return `# ruflo bash completion
# Generated by ruflo completions bash
...
complete -F _claude_flow_completions ruflo
...
function generateZshCompletion(): string {
  return `#compdef ruflo
# ruflo zsh completion
# Generated by ruflo completions zsh
# Add to ~/.zfunc/_ruflo or ~/.zsh/completions/_ruflo
...
_claude_flow() {
    ...
    case $state in
        command)
            _describe -t commands 'ruflo commands' commands
            ;;
        subcommand)
            case $words[2] in
                swarm)
...
function generateFishCompletion(): string {
  return `# ruflo fish completion
# Generated by ruflo completions fish
# Save to ~/.config/fish/completions/ruflo.fish

# Disable file completion by default
complete -c ruflo -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c ruflo -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c ruflo -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c ruflo -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c ruflo -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c ruflo -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c ruflo -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c ruflo -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c ruflo -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c ruflo -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c ruflo -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c ruflo -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c ruflo -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c ruflo -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c ruflo -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c ruflo -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c ruflo -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# ruflo PowerShell completion
# Generated by ruflo completions powershell
...
Register-ArgumentCompleter -Native -CommandName ruflo -ScriptBlock {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'ruflo completions bash > ~/.bash_completion.d/ruflo', description: 'Install bash completions' },
    { command: 'ruflo completions zsh > ~/.zfunc/_ruflo', description: 'Install zsh completions' },
    { command: 'ruflo completions fish > ~/.config/fish/completions/ruflo.fish', description: 'Install fish completions' },
    { command: 'ruflo completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  ruflo completions bash > ~/.bash_completion.d/ruflo'));
    output.writeln(output.dim('  source ~/.bash_completion.d/ruflo'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  ruflo completions zsh > ~/.zfunc/_ruflo'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  ruflo completions fish > ~/.config/fish/completions/ruflo.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  ruflo completions powershell >> $PROFILE'));
